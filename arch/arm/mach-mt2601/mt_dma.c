...
#define NR_CONNSYSDMA_CHANNEL     (1)
#define NR_PDMA_CHANNEL           (4)
#define NR_VFFDMA_CHANNEL         (6)
#define GDMA_START                (0)
#define NR_DMA                    (NR_GDMA_CHANNEL + NR_CONNSYSDMA_CHANNEL + NR_PDMA_CHANNEL + NR_VFFDMA_CHANNEL)
...
/* printk("GDMA fixed address mode requires 8-bytes aligned address\n"); */
	...
	case ALL:
		...
		/*setup coherence bus */
		if (config->cohen) {
			writel((DMA_READ_COHER_BIT | readl(DMA_AXIATTR(DMA_BASE_CH(channel)))),
			       DMA_AXIATTR(DMA_BASE_CH(channel)));
			writel((DMA_WRITE_COHER_BIT | readl(DMA_AXIATTR(DMA_BASE_CH(channel)))),
			       DMA_AXIATTR(DMA_BASE_CH(channel)));
		}

		/*setup security channel */
		if (config->sec) {
			printk("1:GMDA GSEC:%x, ChSEC:%x\n", readl(DMA_GLOBAL_GSEC_EN),
			       readl(DMA_GDMA_SEC_EN(channel)));
			writel((DMA_GSEC_EN_BIT | readl(DMA_GLOBAL_GSEC_EN)), DMA_GLOBAL_GSEC_EN);
			writel((DMA_SEC_EN_BIT | readl(DMA_GDMA_SEC_EN(channel))),
			...
			printk("1:GMDA GSEC:%x, ChSEC:%x\n", readl(DMA_GLOBAL_GSEC_EN),
			       readl(DMA_GDMA_SEC_EN(channel)));
			writel(((~DMA_GSEC_EN_BIT) & readl(DMA_GLOBAL_GSEC_EN)),
			       DMA_GLOBAL_GSEC_EN);
			printk("2:GMDA GSEC:%x, ChSEC:%x\n", readl(DMA_GLOBAL_GSEC_EN),
			       readl(DMA_GDMA_SEC_EN(channel)));
		}
...
/* mt6577_irq_set_polarity(MT6577_GDMA2_IRQ_ID, MT65xx_POLARITY_LOW); */
	...
	emi_mpu_notifier_register(MST_ID_PERI_0, gdma1_check_mpu_violation);
...
void mt65xx_dma_running_status(void)
{
	...
	char *DMA_name[15] = { "G_DMA1", "G_DMA2", "HIF_1", "HIF_2", "SIM_1", "SIM_2", "IrDa Tx/Rx",
		"UART_1 Tx", "UART_1 Rx", "UART_2 Tx", "UART_2 Rx", "UART_3 Tx", "UART_3 Rx",
		"UART_4 Tx", "UART_4 Rx"
	};
