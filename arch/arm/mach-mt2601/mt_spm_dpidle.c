...
wake_reason_t spm_go_to_dpidle(bool cpu_pdn, u8 pwrlevel)
{
	...
	if (pwrlevel > 2) {
		spm_crit2("Hey!! wrong PWR Level: %x", pwrlevel);
		goto RESTORE_IRQ;
	}
	...
	/* pcm_config_dpidle.infra_pdn = false; */

	pcm_config_dpidle.pcm_firmware_addr = PCM_DPIDLE_BASE;
	...
	snapshot_golden_setting(__func__, __LINE__);
