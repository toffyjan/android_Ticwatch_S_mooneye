...
static void init_lcm_registers(void)
{
	...
	data_array[0] = 0x832A1500;
	dsi_set_cmdq(data_array, 1, 1);

	data_array[0] = 0x45301500;
	dsi_set_cmdq(data_array, 1, 1);
...
static void lcm_setbacklight(unsigned int level)
{
	...
	/* level value can't bigger than 0xda(350nits) */
	if ((level > base_level) && (level < 255))
		level = base_level + ((level - base_level) / 3);
...
/* --------------------------------------------------------------------------- */
LCM_DRIVER rm67160_dsi_cmd_lcm_drv = {
	.name = "rm67160_dsi_cmd",
