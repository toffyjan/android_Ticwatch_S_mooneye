...
enum print_line_t mt65xx_mon_print_entry(struct mt65xx_mon_entry *entry,
					 struct trace_iterator *iter)
{
	...
	if (mon_mode_evt != MODE_SCHED_SWITCH) {
		for_each_present_cpu(cpu) {
			trace_seq_printf(s,
					 ...
					 cpu,
					 log_entry->cpu_cnt2[cpu], cpu, log_entry->cpu_cnt3[cpu]);
		}
		...
		trace_seq_printf(s,
				 ...
				 cpu,
				 log_entry->cpu_cnt1[cpu],
				 cpu, log_entry->cpu_cnt2[cpu], cpu, log_entry->cpu_cnt3[cpu]);
	}

	trace_seq_printf(s,
			 "BM_BCNT = %d, BM_TACT = %d, BM_TSCT = %d, ",
			 log_entry->BM_BCNT, log_entry->BM_TACT, log_entry->BM_TSCT);

	trace_seq_printf(s,
			 "BM_WACT = %d, BM_WSCT0 = %d, BM_BACT = %d, ",
			 log_entry->BM_WACT, log_entry->BM_WSCT, log_entry->BM_BACT);

	trace_seq_printf(s, "BM_BSCT = %d, ", log_entry->BM_BSCT);

	trace_seq_printf(s,
			 "BM_TSCT2 = %d, BM_WSCT2 = %d, ",
			 log_entry->BM_TSCT2, log_entry->BM_WSCT2);

	trace_seq_printf(s,
			 "BM_TSCT3 = %d, BM_WSCT3 = %d, ",
			 log_entry->BM_TSCT3, log_entry->BM_WSCT3);

	trace_seq_printf(s,
			 "BM_WSCT4 = %d, BM_TPCT1 = %d, ",
			 log_entry->BM_WSCT4, log_entry->BM_TPCT1);
	...
	trace_seq_printf(s,
			 ...
			 log_entry->DRAMC_Interbank, log_entry->DRAMC_Idle);
...
/* *(volatile unsigned int *)0xF702A000 |= 0x1000; */
	...
	BM_SetMonitorCounter(1, BM_MASTER_MMSYS,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
	BM_SetMonitorCounter(2, BM_MASTER_AP_MCU,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
	BM_SetMonitorCounter(3, BM_MASTER_MD_MCU | BM_MASTER_MD_HW,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
	BM_SetMonitorCounter(4, BM_MASTER_PERI,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
