...
static int mpu_dev_open(struct inode *inode, struct file *filp)
{
	int ret;
...
static int mpu_dev_release(struct inode *inode, struct file *filp)
{
	mtk_mon->deinit();
...
static long mpu_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	u32 index, ret;
	...
	case IOCTL_PRINTLOG:
		...
		ret =
		    copy_to_user((void *)arg, (const void *)&mtk_mon->log_buff[index],
