...
#if 0
	for (temp_address = v_pic0_addr; temp_address <= (v_pic0_addr + (w * h * pitch));
	     temp_address += 4) {
...
/* // polling if we get mutex0 */
	while ((spm_read(DISP_MUTEX_BASE + 0x24) & 0x2) != 0x2) {
		mdelay(500);
	}
	...
	while (do_not_change_it) {
		...
		case 0:
			...
			spm_write(SPM_POWER_ON_VAL1,
				  (spm_read(SPM_POWER_ON_VAL1) & 0xFFFFFFCF) | 0x220);
...
void spm_mcdi_LDVT_mcdi(void)
{
...
#if 1
	{
		...
		mcdi_error =
		    request_gpt(GPT4, GPT_ONE_SHOT, GPT_CLK_SRC_SYS, GPT_CLK_DIV_1, 0, NULL,
				GPT_NOAUTOEN);
		if (mcdi_error != 0) {
	...
	if (IS_ERR(mcdi_task_0) || IS_ERR(mcdi_task_1)) {
		clc_notice("Unable to start kernel thread(0x%x, 0x%x)./n",
			   (u32) IS_ERR(mcdi_task_0), (u32) IS_ERR(mcdi_task_1));
		mcdi_task_0 = NULL;
		mcdi_task_1 = NULL;
	}
