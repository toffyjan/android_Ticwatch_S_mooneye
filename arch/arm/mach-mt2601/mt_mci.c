...
void mci_dump_structure(void)
{
	dbg_printk("[MCI] mci_confi:%d,%d,%d,%d,%d\n", mci_conf.mci_write_skip,
		   mci_conf.mci_write_snoop_early_resp, mci_conf.mci_write_snoop_outstanding,
		   mci_conf.mci_bar, mci_conf.mci_snoop);
}
...
int dma_test_exit(unsigned int cohen, unsigned cacheable)
{
	if (cacheable) {
		...
		dma_unmap_single(NULL, dst_array_p,
				 sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int),
				 DMA_TO_DEVICE);
		dma_unmap_single(NULL, src_array_p,
				 sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int),
				 DMA_FROM_DEVICE);
		/* kfree(dst_array_v); */
		/* kfree(src_array_v); */
	...
	for (i = 0; i < BUFF_LEN / sizeof(unsigned int); i++)
		if (dst_array_v[i] != PATTERN3) {
			printk("DMA ERROR at Address %x,%x != %x, (%x=%x)\n", &dst_array_v[i],
			       dst_array_v[i], PATTERN3, &src_array_v[i], src_array_v[i]);
			result = -1;
		}
...
static int dma_test_init(unsigned int cohen, unsigned cacheable)
{
	...
	if (cacheable) {
		dst_array_v =
		    kzalloc(sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int), GFP_ATOMIC);
		src_array_v =
		    kzalloc(sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int), GFP_ATOMIC);
	} else {
		dst_array_v =
		    dma_alloc_coherent(NULL, sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int),
				       &dst_array_p, GFP_KERNEL);
		src_array_v =
		    dma_alloc_coherent(NULL, sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int),
				       &src_array_p, GFP_KERNEL);
	}
	...
	if (cacheable) {
		...
		dst_array_p =
		    dma_map_single(NULL, dst_array_v,
				   sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int),
				   DMA_TO_DEVICE);
		src_array_p =
		    dma_map_single(NULL, src_array_v,
				   sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int),
				   DMA_FROM_DEVICE);
...
static void ts_complex_mem_test(void)
{
...
static int ts_ACELite_MCU_snoop_read_cache_hit(void)
{
	...
	dst_array_p =
	    dma_map_single(NULL, dst_array_v,
			   sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int), DMA_TO_DEVICE);
	src_array_p =
	    dma_map_single(NULL, src_array_v,
			   sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int), DMA_FROM_DEVICE);
	if (dma_mapping_error(NULL, dst_array_p) || dma_mapping_error(NULL, src_array_p)) {
		printk("ERROR Couldn't map DMA\n");
		return -1;
	}
 ...
 */
static ssize_t mci_dvt_store(struct device_driver *driver, const char *buf, size_t count)
{
	char *p = (char *)buf;
	unsigned int num;
