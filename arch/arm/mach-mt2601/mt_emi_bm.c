...
{				/* CM [20121101] wait for implement */
...
int BM_GetTransTypeCount(const unsigned int counter_num)
{
	return (counter_num < 1
		|| counter_num >
		BM_COUNTER_MAX) ? BM_ERR_WRONG_REQ : readl(EMI_TTYPE1 + (counter_num - 1) * 8);
}
			 ...
			 const unsigned int trans_type)
{
	...
	if (counter_num == 1) {
		addr = EMI_BMEN;
		value =
		    (readl(addr) & ~(iMask << 16)) | ((trans_type & 0xFF) << 24) | ((master & 0xFF)
										    << 16);
	} else {
...
int BM_SetIDSelect(const unsigned int counter_num, const unsigned int id, const unsigned int enable)
{
	...
	value =
	    (readl(EMI_BMID5) & ~(1 << (counter_num - 1 + 7))) | (enable << (counter_num - 1 + 7));
	mt_reg_sync_writel(value, EMI_BMID5);

	return BM_REQ_OK;
}

int BM_SetUltraHighFilter(const unsigned int counter_num, const unsigned int enable)
{				/* CM [20121102] wait for implement */
	unsigned int value;
