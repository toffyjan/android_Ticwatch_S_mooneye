...
static void usb_write_fifo(u16 len, const u8 *src, u8 ep_num)
{
	...
	if (likely((0x01 & (unsigned long) src) == 0)) {
		...
		if ((0x02 & (unsigned long) src) == 0) {
			if (len >= 4) {
				writesl(USB_BASE+fifo, src + index, len >> 2);
				index += len & ~0x03;
			}
			if (len & 0x02) {
			...
			if (len >= 2) {
				writesw(USB_BASE+fifo, src + index, len >> 1);
				index += len & ~0x01;
			}
		}
		if (len & 0x01)
...
static void usb_read_fifo(u16 len, u8 *dst, u8 ep_num)
{
	...
	if (likely((0x01 & (unsigned long) dst) == 0)) {
		...
		if ((0x02 & (unsigned long) dst) == 0) {
			if (len >= 4) {
				readsl(USB_BASE+fifo, dst, len >> 2);
				index = len & ~0x03;
			}
			if (len & 0x02) {
			...
			if (len >= 2) {
				readsw(USB_BASE+fifo, dst, len >> 1);
				index = len & ~0x01;
			}
		}
		if (len & 0x01)
