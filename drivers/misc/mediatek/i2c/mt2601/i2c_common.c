...
static ssize_t set_config(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("%s\n", buf);
	if (sscanf(buf, "%d %d %d %d %d %d %d %d %d %d %d %d %1023s", &bus_id, &address, &operation, &trans_mode, &trans_stop, &speed_mode, &pushpull_mode, &query_mode, &timing, &trans_num, &trans_auxlen, &dir, data_buffer)) {
