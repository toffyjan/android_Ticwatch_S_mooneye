...
inline static void __write_fcr_register(struct mtk_uart *uart, u32 data)
{
    ...
    mt_reg_sync_writel(data, UART_FCR);
}
...
inline static void __set_fcr_register(struct mtk_uart *uart, u32 mask)
{
    ...
    mt_reg_sync_writel(new_setting, UART_FCR);
}
...
{
    mt_reg_sync_writel(VFF_TX_INT_EN_B, VFF_INT_EN(uart->tx_vfifo->base));
}
/*---------------------------------------------------------------------------*/
void mtk_uart_vfifo_disable_tx_intr(struct mtk_uart *uart)
{
    mt_reg_sync_writel(0x00, VFF_INT_EN(uart->tx_vfifo->base));
}
/*---------------------------------------------------------------------------*/
void mtk_uart_vfifo_clear_tx_intr(struct mtk_uart_vfifo *vfifo)
{
    mt_reg_sync_writel(0x00, VFF_INT_FLAG(vfifo->base));
}
/*---------------------------------------------------------------------------*/
void mtk_uart_vfifo_clear_rx_intr(struct mtk_uart_vfifo *vfifo)
{
    mt_reg_sync_writel(0x03, VFF_INT_FLAG(vfifo->base));
}
/*---------------------------------------------------------------------------*/
void mtk_uart_vfifo_enable_rx_intr(struct mtk_uart *uart)
{
    mt_reg_sync_writel(VFF_RX_INT_EN0_B|VFF_RX_INT_EN1_B, VFF_INT_EN(uart->rx_vfifo->base));
}
/*---------------------------------------------------------------------------*/
void mtk_uart_vfifo_disable_rx_intr(struct mtk_uart *uart)
{
    mt_reg_sync_writel(0x00, VFF_INT_EN(uart->rx_vfifo->base));
}
/*---------------------------------------------------------------------------*/
...
void mtk_uart_vfifo_write_byte(struct mtk_uart *uart, unsigned int byte)
{
    ...
    mt_reg_sync_writeb((unsigned char)byte, addr);
    mb();                                              /* make sure write point updated after VFIFO written. */
    save_tx_raw_data(uart, addr);
    if ((wpt&0xffff) == (UART_READ32(VFF_LEN(base))-1))
	mt_reg_sync_writel((~wpt)&0x10000, VFF_WPT(base));
    else
	mt_reg_sync_writel(wpt+1, VFF_WPT(base));
}
/*---------------------------------------------------------------------------*/
unsigned int mtk_uart_vfifo_read_byte(struct mtk_uart *uart)
{
	...
	if (UART_READ16(VFF_RPT(base)) == (UART_READ32(VFF_LEN(base))-1))
		mt_reg_sync_writel(~(UART_READ32(VFF_RPT(base)))&0x10000, VFF_RPT(base));
	else
		mt_reg_sync_writel(UART_READ32(VFF_RPT(base))+1, VFF_RPT(base));
...
void mtk_uart_tx_vfifo_flush(struct mtk_uart *uart, int timeout)
{
	...
	if (UART_READ32(VFF_FLUSH(base)) == 0) {
	    mt_reg_sync_writel(VFF_FLUSH_B, VFF_FLUSH(base));
	    if (!timeout)
...
static void mtk_uart_dma_vfifo_rx_tasklet_str(unsigned long arg)
{
    ...
    dsb();                                                  /* make sure read point updated after VFIFO read. */
    mt_reg_sync_writel(txreg, VFF_RPT(base));
				  ...
				  struct mtk_uart_dma *dma)
{
	...
	base = dma->vfifo->base;
	mt_reg_sync_writel(dma->vfifo->dmahd, VFF_ADDR(base));
	mt_reg_sync_writel(dma->vfifo->trig, VFF_THRE(base));
	mt_reg_sync_writel(dma->vfifo->size, VFF_LEN(base));

	if (dma->vfifo->type == UART_RX_VFIFO)
	    /* reg_sync_writel(VFF_RX_INT_EN0_B, VFF_INT_EN(base)); */
	    mt_reg_sync_writel(VFF_RX_INT_EN0_B|VFF_RX_INT_EN1_B, VFF_INT_EN(base));
	dsb();
    }
}
/*---------------------------------------------------------------------------*/
int mtk_uart_dma_start(struct mtk_uart *uart, struct mtk_uart_dma *dma)
{
	...
	base = dma->vfifo->base;
	mt_reg_sync_writel(VFF_INT_FLAG_CLR_B, VFF_INT_FLAG(base));
	mt_reg_sync_writel(VFF_EN_B, VFF_EN(base));
...
void mtk_uart_stop_dma(struct mtk_uart_dma *dma)
{
	...
	mt_reg_sync_writel(VFF_FLUSH_CLR_B, VFF_FLUSH(base));
	...
	mt_reg_sync_writel(VFF_STOP_B, VFF_STOP(base));
	...
	mt_reg_sync_writel(VFF_STOP_CLR_B, VFF_STOP(base));

	mt_reg_sync_writel(VFF_INT_EN_CLR_B, VFF_INT_EN(base));
	mt_reg_sync_writel(VFF_INT_FLAG_CLR_B, VFF_INT_FLAG(base));
    } else {
...
void mtk_uart_reset_dma(struct mtk_uart_dma *dma)
{
	...
	mt_reg_sync_writel(0, VFF_ADDR(base));
	mt_reg_sync_writel(0, VFF_THRE(base));
	mt_reg_sync_writel(0, VFF_LEN(base));
	/*set warm_rst as 1 -> wait until en is 0*/
	mt_reg_sync_writel(VFF_WARM_RST_B, VFF_RST(base));
	while (UART_READ32(VFF_EN(base)));

	/* Reset write point for tx dma */
	if (dma->mode == UART_TX_VFIFO_DMA) {
		mt_reg_sync_writel(0, VFF_WPT(base));
	} else if (dma->mode == UART_RX_VFIFO_DMA) {
		mt_reg_sync_writel(0, VFF_RPT(base));
	}
    } else {
	MSG(ERR, "unknown mode: %d\n", dma->mode);
    }
				      ...
				      int rx_level)
{
    ...
    mt_reg_sync_writel(tmp1, UART_LCR);
    spin_unlock_irqrestore(&mtk_console_lock, flags);
...
void mtk_uart_set_auto_baud(struct mtk_uart *uart)
{
    ...
    {
    case MTK_SYSCLK_13:
	mt_reg_sync_writel(UART_AUTOBADUSAM_13M, UART_AUTOBAUD_SAMPLE);
	break;
    case MTK_SYSCLK_26:
	 mt_reg_sync_writel(UART_AUTOBADUSAM_26M, UART_AUTOBAUD_SAMPLE);
	break;
    case MTK_SYSCLK_52:
	mt_reg_sync_writel(UART_AUTOBADUSAM_52M, UART_AUTOBAUD_SAMPLE);
	break;
    default:
    ...
    mt_reg_sync_writel(0x01, UART_AUTOBAUD_EN); /* Enable Auto Baud */
    return;
}
/*---------------------------------------------------------------------------*/
static void mtk_uart_cal_baud(struct mtk_uart *uart, int baudrate, int highspeed)
{
	...
	mt_reg_sync_writel(0x03, UART_RATE_FIX_AD);
    } else{				/* >1M, Using 65 clock */
	uartclk = uart->sysclk;
	mt_reg_sync_writel(0x00, UART_RATE_FIX_AD);
    }
    if (3 == highspeed)
	...
	mt_reg_sync_writel(0x00, UART_HIGHSPEED); /*divider is 16*/
	divisor   = (uartclk >> 4)/(u32)baudrate;
	remainder = (uartclk >> 4)%(u32)baudrate;
	if (remainder >= (u32)(baudrate*8))
	    divisor += 1;
	mt_reg_sync_writel(lcr|UART_LCR_DLAB, UART_LCR);
	mt_reg_sync_writel((divisor&0xFF), UART_DLL);
	mt_reg_sync_writel(((divisor>>8) & 0xFF), UART_DLH);
	mt_reg_sync_writel(lcr, UART_LCR);
    } else if (highspeed == 1) {
	/* uartclk = uart->sysclk; */
	/* reg_sync_writel(0x00, UART_RATE_FIX_AD); */
	mt_reg_sync_writel(0x01, UART_HIGHSPEED); /*divider is 8*/
	...
	/* uartclk = uart->sysclk; */
	/* reg_sync_writel(0x00, UART_RATE_FIX_AD); */
	mt_reg_sync_writel(0x02, UART_HIGHSPEED); /*divider is 4*/
	divisor   = (uartclk >> 2)/(u32)baudrate;
	remainder = (uartclk >> 2)%(u32)baudrate;
	if (remainder >= (u32)(baudrate*2))
	    divisor += 1;
	mt_reg_sync_writel(lcr|UART_LCR_DLAB, UART_LCR);
	mt_reg_sync_writel((divisor&0x00FF), UART_DLL);
	mt_reg_sync_writel(((divisor>>8) & 0x00FF), UART_DLH);
	mt_reg_sync_writel(lcr, UART_LCR);
    } else if (highspeed == 3) {
	...
	/* uartclk = uart->sysclk; */
	/* reg_sync_writel(0x00, UART_RATE_FIX_AD); */
	mt_reg_sync_writel(0x03, UART_HIGHSPEED);
	...
	mt_reg_sync_writel(fraction_L_mapping[fraction], UART_FRACDIV_L);
	mt_reg_sync_writel(fraction_M_mapping[fraction], UART_FRACDIV_M);
    #else
	...
	mt_reg_sync_writel(lcr|UART_LCR_DLAB, UART_LCR);
	mt_reg_sync_writel((high_div & 0x00FF), UART_DLL);
	mt_reg_sync_writel(((high_div>>8) & 0x00FF), UART_DLH);
	mt_reg_sync_writel(lcr, UART_LCR);
	mt_reg_sync_writel(sample_count, UART_SAMPLE_COUNT);
	mt_reg_sync_writel(sample_point, UART_SAMPLE_POINT);
	/*
	 ...
	 */
	if (baudrate >= 3000000)
	    mt_reg_sync_writel(0x12, UART_GUARD);
    }
    spin_unlock_irqrestore(&mtk_console_lock, flags);
...
void mtk_uart_set_flow_ctrl(struct mtk_uart *uart, int mode)
{
    ...
    switch (mode) {
    case UART_FC_NONE:
	mt_reg_sync_writel(UART_ESCAPE_CH, UART_ESCAPE_DAT);
	mt_reg_sync_writel(0x00, UART_ESCAPE_EN);
	mt_reg_sync_writel(0xbf, UART_LCR);
	old = UART_READ32(UART_EFR);
	old &= ~(UART_EFR_AUTO_RTSCTS|UART_EFR_XON12_XOFF12);
	mt_reg_sync_writel(old, UART_EFR);
	mt_reg_sync_writel(tmp, UART_LCR);
	...
	old = UART_READ32(UART_EFR);
	mt_reg_sync_writel(old | UART_EFR_AUTO_RTSCTS, UART_EFR);
	mt_reg_sync_writel(tmp, UART_LCR);
	...
	mt_reg_sync_writel(UART_ESCAPE_CH, UART_ESCAPE_DAT);
	mt_reg_sync_writel(0x01, UART_ESCAPE_EN);
	mt_reg_sync_writel(0xbf, UART_LCR);
	...
	old = UART_READ32(UART_EFR);
	mt_reg_sync_writel(old | UART_EFR_XON1_XOFF1, UART_EFR);
	mt_reg_sync_writel(START_CHAR(uart->port.state->port.tty), UART_XON1);
	mt_reg_sync_writel(STOP_CHAR(uart->port.state->port.tty), UART_XOFF1);
	mt_reg_sync_writel(tmp, UART_LCR);
...
void mtk_uart_config(struct mtk_uart *uart, int datalen, int stop, int parity)
{
    ...
    mt_reg_sync_writel(val, UART_LCR);
}
/*---------------------------------------------------------------------------*/
...
{
    u32 base = uart->base;
    mt_reg_sync_writel(byte, UART_THR);
}
/*---------------------------------------------------------------------------*/
...
void mtk_uart_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
    ...
    mt_reg_sync_writel(val, UART_MCR);
...
void mtk_uart_stop_rx(struct uart_port *port)
{
	 ...
	 */
	mtk_uart_disable_intrs(uart, UART_IER_ERBFI);
	mt_reg_sync_writel(VFF_INT_EN_CLR_B, VFF_INT_EN(dma->vfifo->base));
	atomic_set(&dma->free, 1);
	complete(&dma->done);
#endif
    }
