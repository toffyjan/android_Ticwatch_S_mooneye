...
#include "mali_kernel_common.h"
   ...
   #define MALI_OSK_SPINLOCK_MAX 64
   extern int     mali_osk_spinlock_key_num;
   extern struct lock_class_key mali_spinlock_keys[];

   extern int     mali_osk_spinlock_irq_key_num;
   extern struct  lock_class_key mali_spinlock_irq_keys[];
#endif
...
#define _mali_osk_locks_debug_remove(x) do {} while (0)
	...
	static inline _mali_osk_spinlock_t *_mali_osk_spinlock_init(_mali_osk_lock_flags_t flags, _mali_osk_lock_order_t order)
	{
		...
		#ifdef CONFIG_PROVE_LOCKING
	/* The solution for the false alarm of dynamic init locks */
	if (mali_osk_spinlock_key_num < MALI_OSK_SPINLOCK_MAX)
	{
	    lockdep_set_class(&lock->spinlock, &mali_spinlock_keys[mali_osk_spinlock_key_num++]);
	}
	else
	{
	    printk(KERN_WARNING "Mali: Not enough spin lock key\n");
	}
		#endif
	...
	static inline _mali_osk_spinlock_irq_t *_mali_osk_spinlock_irq_init(_mali_osk_lock_flags_t flags, _mali_osk_lock_order_t order)
	{
		...
		#ifdef CONFIG_PROVE_LOCKING
	/* The solution for the false alarm of dynamic init locks */
	if (mali_osk_spinlock_irq_key_num < MALI_OSK_SPINLOCK_MAX)
	{
	    lockdep_set_class(&lock->spinlock, &mali_spinlock_irq_keys[mali_osk_spinlock_irq_key_num++]);
	}
	else
	{
	    printk(KERN_WARNING "Mali: Not enough spin lock irq key\n");
	}
		#endif
		_mali_osk_locks_debug_init((struct _mali_osk_lock_debug_s *)lock, flags, order);
		return lock;
