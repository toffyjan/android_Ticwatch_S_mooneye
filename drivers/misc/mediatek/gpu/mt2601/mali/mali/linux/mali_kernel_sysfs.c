...
static int proc_dbginfo_show(struct seq_file *m, void *v)
{
    char buf[1024];

    mali_platform_dump_all_log(buf);
...
static int proc_dbginfo_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_dbginfo_show, NULL);
}

static const struct file_operations proc_dbginfo_operations = {
    .open    = proc_dbginfo_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release,
};
