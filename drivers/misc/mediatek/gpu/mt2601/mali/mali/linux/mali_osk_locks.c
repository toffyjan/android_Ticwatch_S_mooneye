...
static const char * const lock_order_to_string(_mali_osk_lock_order_t order);
   ...
   int     mali_osk_spinlock_key_num = 0;
   struct  lock_class_key mali_spinlock_keys[MALI_OSK_SPINLOCK_MAX];

   int     mali_osk_spinlock_irq_key_num = 0;
   struct  lock_class_key mali_spinlock_irq_keys[MALI_OSK_SPINLOCK_MAX];
#endif
