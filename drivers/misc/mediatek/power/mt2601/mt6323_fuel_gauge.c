 ...
 * Description:
 * ------------
 *   This Module defines functions of 6323 fuel gauge
 *
 * Author:
 * -------
 * -------
...
#define UNIT_FGCHARGE      (21961412)     /* 0.021961412 uAh //6323 */
...
int volt_mode_update_time_out = 6;
...
void get_hw_chip_diff_trim_value(void)
{
    ...
    #if 0
    reg_val = upmu_get_reg_value(reg_num);
...
static int FGbatteryIndex;
static int FGbatteryVoltageSum;
...
/* // platform_driver API */
/* ///////////////////////////////////////////////////////////////////////////////////////// */
static int mt6323_fgadc_probe(struct platform_device *dev)
{
...
#else
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6323 FGADC driver probe!!\n");
...
static int mt6323_fgadc_remove(struct platform_device *dev)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6323 FGADC driver remove!!\n");

    return 0;
}

static void mt6323_fgadc_shutdown(struct platform_device *dev)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6323 FGADC driver shutdown!!\n");
}

static int mt6323_fgadc_suspend(struct platform_device *dev, pm_message_t state)
{
...
static int mt6323_fgadc_resume(struct platform_device *dev)
{
#if defined(CONFIG_POWER_EXT)
...
struct platform_device MT6323_fgadc_device = {
	.name                = "mt6323-fgadc",
	.id                  = -1,
};

static struct platform_driver mt6323_fgadc_driver = {
    .probe        = mt6323_fgadc_probe,
    .remove       = mt6323_fgadc_remove,
    .shutdown     = mt6323_fgadc_shutdown,
    /* #ifdef CONFIG_PM */
    .suspend      = mt6323_fgadc_suspend,
    .resume       = mt6323_fgadc_resume,
    /* #endif */
    .driver       = {
	.name = "mt6323-fgadc",
    },
...
static int __init mt6323_fgadc_init(void)
{
    int ret;

    ret = platform_device_register(&MT6323_fgadc_device);
    if (ret) {
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6323_fgadc_driver] Unable to device register(%d)\n", ret);
	return ret;
    }

    ret = platform_driver_register(&mt6323_fgadc_driver);
    if (ret) {
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6323_fgadc_driver] Unable to register driver (%d)\n", ret);
	return ret;
    }

    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6323_fgadc_driver] Initialization : DONE\n");

    return 0;

}

static void __exit mt6323_fgadc_exit(void)
{
}
module_init(mt6323_fgadc_init);
module_exit(mt6323_fgadc_exit);

MODULE_AUTHOR("James Lo");
MODULE_DESCRIPTION("MT6323 FGADC Device Driver");
MODULE_LICENSE("GPL");
#endif
