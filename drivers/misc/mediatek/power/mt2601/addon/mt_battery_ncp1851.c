...
void pchr_turn_on_charging_ncp1851(void)
{
  ...
  {
      pr_debug("[BATTERY:ncp1851] In meta or advanced meta mode, disable charging.\r\n");
      pchr_turn_off_charging_ncp1851();
  }
	else
	{
	...
	if (Enable_BATDRV_LOG == 1) {
	    pr_debug("[BATTERY:ncp1851] select_charging_curret_bcct !\n");
	}
    }
    else
    {
	...
	if (Enable_BATDRV_LOG == 1) {
	    pr_debug("[BATTERY:ncp1851] select_charging_curret_ncp1851 !\n");
	}
    }
...
/* g_bat_temperature_pre = 22; // MT6320 E1 workaround */
    ...
    if (Enable_BATDRV_LOG == 1)
    {
	pr_debug("[BATTERY:ncp1851] Battery Protection Check !!\n");
    }
    ...
    {
	pr_debug("[BATTERY:ncp1851] JEITA : fail\n");
    ...
    {
	pr_debug("[BATTERY:ncp1851] Battery Under Temperature or NTC fail !!\n\r");
	BMT_status.bat_charging_state = CHR_ERROR;
	return PMU_STATUS_FAIL;
    }
    #endif
    ...
    {
	pr_debug("[BATTERY:ncp1851] Battery Over Temperature !!\n\r");
	BMT_status.bat_charging_state = CHR_ERROR;
	return PMU_STATUS_FAIL;
    }
#endif
	...
	{
	    pr_debug("[BATTERY:ncp1851]Charger under voltage!!\r\n");
	    BMT_status.bat_charging_state = CHR_ERROR;
	    return PMU_STATUS_FAIL;
	}
	#endif
	...
	{
	    pr_debug("[BATTERY:ncp1851]Charger over voltage!!\r\n");
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	{
	    if (Enable_BATDRV_LOG >= 1) {
		pr_debug("[BATTERY:ncp1851] double check Battery Re-charging !!\n");
	    }
	    BMT_status.bat_full = KAL_FALSE;
	    g_bat_full_user_view = KAL_TRUE;
	    g_HW_Charging_Done = 0;
...
void check_battery_exist(void)
{
	...
	{
	    pr_debug("[BATTERY] Battery is not exist, power off NCP1851 and system (%d)\n", baton_count);
	    pchr_turn_off_charging_ncp1851();
            arch_reset(0, NULL);
	}
    }
#else
    pr_debug("[BATTERY:ncp1851] Disable check battery exist for SMT\n");
#endif
}
...
void BAT_thread_ncp1851(void)
{
	...
	if (upmu_is_chr_det() == KAL_TRUE)
	{
	    ...
	    pr_debug("[BATTERY:ncp1851] ret_val=%d, g_bat_full_user_view=%d\n", ret_val, g_bat_full_user_view);
	}
        boot_check_once = 0;
    }
