...
#define BLS_DBG(fmt, args...) if (bls_dbg_log) pr_warn("[BLS]"fmt, ##args)	/* default off, use "adb shell "echo dbg_log:1 > sys/kernel/debug/dispsys" to enable */
#define BLS_MSG(fmt, args...) pr_warn("[BLS]"fmt, ##args)	/* default on, important msg, not err */
#define BLS_ERR(fmt, args...) pr_err("[BLS]error:"fmt, ##args)	/* default on, err msg */
...
void disp_bls_update_gamma_lut(void)
{
	...
	BLS_DBG("disp_bls_update_gamma_lut!\n");
...
void disp_bls_init(unsigned int srcWidth, unsigned int srcHeight)
{
	...
	if (cust_led_list) {
		...
		if ((strcmp(cust->name, "lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
			...
			if (config_data->clock_source >= 0 && config_data->clock_source <= 1) {
				unsigned int regVal = DISP_REG_GET(CLK_MUX_SEL);
				...
				BLS_DBG("disp_bls_init : CLK_MUX_SEL 0x%x => 0x%x\n", regVal, DISP_REG_GET(CLK_MUX_SEL));
			}
	...
	BLS_MSG("disp_bls_init : BLS_EN=0x%x, PWM_DUTY=%d, PWM_DUTY_RD=%d, CG=0x%x, %d, %d\n",
		...
		clock_is_on(MT_CG_MDP_BLS_26M_SW_CG),
...
int disp_bls_config(void)
{
		...
		if ((strcmp(cust->name, "lcd-backlight") == 0) && (cust->mode == MT65XX_LED_MODE_CUST_BLS_PWM)) {
			...
			if (config_data->clock_source >= 0 && config_data->clock_source <= 1) {
				...
				if (config_data->clock_source == 0)
					...
					clkmux_sel(MT_CLKMUX_PWM_MM_MUX_SEL, MT_CG_UPLL_D12, "DISP_PWM");
				BLS_DBG("disp_bls_config : CLK_MUX_SEL 0x%x => 0x%x\n", regVal, DISP_REG_GET(CLK_MUX_SEL));
			}
			gPWMDiv = (config_data->div == 0) ? PWM_DEFAULT_DIV_VALUE : config_data->div;
			gPWMDiv &= 0x3FF;
	...
	if (!clock_is_on(MT_CG_MDP_BLS_26M_SW_CG) || !gBLSPowerOn)
		ASSERT(0);
...
void disp_bls_config_full(unsigned int width, unsigned int height)
{
	...
	unsigned int dither_en = 0x00000000;

#ifdef CONFIG_MTK_DITHERING_SUPPORT
...
/* output RGB888 */
	if (dither_bpp == 16) {	/* 565 */
		...
		DISP_REG_SET(DISP_REG_BLS_DITHER(0), dither_en);
...
int disp_bls_set_backlight(unsigned int level)
{
	...
	if (level) {
		if (!clock_is_on(MT_CG_DISP_BLS_SW_CG)) {
			...
			isBlsEnable = FALSE;
			enable_clock(MT_CG_DISP_BLS_SW_CG, "BLS");
			if (clock_is_on(MT_CG_MDP_BLS_26M_SW_CG))
				DISP_REG_SET(DISP_REG_BLS_EN, 0x10000);
			else
				DISP_REG_SET(DISP_REG_BLS_EN, 0x0);
			disp_bls_init(fb_width, fb_height);
		}

		if (!clock_is_on(MT_CG_PWM_MM_SW_CG))
			enable_clock(MT_CG_PWM_MM_SW_CG, "BLS");
		if (!clock_is_on(MT_CG_MDP_BLS_26M_SW_CG))
			enable_clock(MT_CG_MDP_BLS_26M_SW_CG, "BLS");
	}
#ifdef USE_DISP_BLS_MUTEX
	disp_bls_get_mutex();
#else
	...
	if (!level) {
		if (clock_is_on(MT_CG_MDP_BLS_26M_SW_CG))
			disable_clock(MT_CG_MDP_BLS_26M_SW_CG, "BLS");
		if (clock_is_on(MT_CG_PWM_MM_SW_CG))
			disable_clock(MT_CG_PWM_MM_SW_CG, "BLS");
	}

	if (!isBlsEnable)
		disable_clock(MT_CG_DISP_BLS_SW_CG, "BLS");
	mutex_unlock(&backlight_mutex);
	return 0;
}
#else
