						   ...
						   const unsigned int mvaRegionStart)
{
	...
	if (!(startIdx != 0             /* startIdx is not NULL */
		  ...
		  &&(moduleGraph[startIdx] == eModuleID)
		  &&(nr == nrRequire)
		 )
	   )
	{
	...
	if (pMlock_cnt[pfn])
	{
		...
		if (pMlock_cnt[pfn] == 0)
		{
			...
			if (PageMlocked(page) == 1)
			{
				...
				dump_page(page, " Can't munlock page:\n");
			}
		}
	}
	else
			...
			if ((vma->vm_flags) & VM_PFNMAP)
			{
				...
				if (bufEnd > vma->vm_end)
				{
					M4UMSG("error: page_num=%d,module=%s, va=0x%x, size=0x%x, vm_flag=0x%x\n",
						   page_num, m4u_get_module_name(eModuleID), BufAddr, BufSize, (unsigned int)vma->vm_flags);
				...
				M4UMSG("alloc_mva VM_PFNMAP module=%s, va=0x%x, size=0x%x, vm_flag=0x%x\n",
					   m4u_get_module_name(eModuleID), BufAddr, BufSize, (unsigned int)vma->vm_flags);
				up_read(&current->mm->mmap_sem);
			}
			else
			{
...
int m4u_dma_cache_maint(const M4U_MODULE_ID_ENUM eModuleID, const void *start, const size_t size, const int direction)
{
	...
	if (L1_CACHE_SYNC_BY_RANGE_ONLY)
	{
		...
		{
			dma_map_single(NULL, start, size, direction);
		}
	...
	if (L1_CACHE_SYNC_BY_RANGE_ONLY)
	{
		...
		{
			dma_unmap_single(NULL, start, size, direction);
		}
	}
...
static int __m4u_cache_sync_kernel(const void *start, size_t size, int direction)
{
	...
	{
		dma_map_single(NULL, (void *)start, size, DMA_TO_DEVICE);
	}
	else if (direction == DMA_FROM_DEVICE) /* invalid */
	{
		dma_unmap_single(NULL, (dma_addr_t)start, size, DMA_FROM_DEVICE);
	}
...
#define M4U_PAGE_TABLE_ALIGN (((PT_TOTAL_ENTRY_NUM*sizeof(unsigned int) - 1) > 0xffff) ? (PT_TOTAL_ENTRY_NUM*sizeof(unsigned int) - 1) : 0xffff)
...
static char *m4u_get_module_name(const M4U_MODULE_ID_ENUM moduleID)
{
	switch (moduleID)
	{
		case M4U_CLNTMOD_MDP:
			return "MDP";
		case M4U_CLNTMOD_DISP:
			return "DISP";
		case M4U_CLNTMOD_VIDEO:
			return "VIDEO";
...
static char *m4u_get_port_name(const M4U_PORT_ID_ENUM portID)
{
	switch (portID)
	{
		...
		case M4U_PORT_VENCMC:
			return "VENCMC";
		case M4U_PORT_VENC_CDMA_VDEC_CDMA:
			return "VENC_CDMA_VDEC_CDMA";
		case M4U_PORT_VENC_REC_VDEC_WDMA:
			return "VENC_REC_VDEC_WDMA";
		default:
 ...
 *    None.
 ****************************************************************************/
static void m4u_memory_usage(unsigned long value)
{
	unsigned int i = 0;
