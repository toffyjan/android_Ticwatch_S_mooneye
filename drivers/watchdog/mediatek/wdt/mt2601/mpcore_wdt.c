...
do {\
	printk(KERN_DEBUG DEBUG_INFO fmt, ##arg);\
	printk(KERN_DEBUG DEBUG_INFO"%s, %d\n", __func__, __LINE__);\
} while (0)
#else
...
static irqreturn_t mpcore_wdt_fire(int irq, void *arg)
{
	...
	printk(KERN_DEBUG DEBUG_INFO"Triggered :cpu-%d\n", get_cpu());
...
static void mpcore_wdt_keepalive(struct mpcore_wdt *wdt)
{
	...
	printk(KERN_DEBUG "WDT mpcore_wdt_keepalive:cpu-%d: count:%ld. prescale:%ld, mpcore_margin:%d,TWD_WDOG_LOAD:%x\n", cpu, count, prescale, mpcore_margin, readl(wdt->base + TWD_WDOG_LOAD));
...
static void mpcore_wdt_start(struct mpcore_wdt *wdt)
{
	...
	spin_unlock_irqrestore(&wdt_lock, flags);
	printk(KERN_DEBUG "WDT mpcore_wdt_start:cpu-%d,control: 0x%x, count:0x%x, load:0x%x\n", cpu, readl(wdt->base + TWD_WDOG_CONTROL), readl(wdt->base + TWD_WDOG_COUNTER), readl(wdt->base + TWD_WDOG_LOAD));
	/* mpcore_wdt_print("cpu-%d,control: 0x%x, count:0x%x, load:0x%x\n", cpu, readl(wdt->base + TWD_WDOG_CONTROL), readl(wdt->base + TWD_WDOG_COUNTER), readl(wdt->base + TWD_WDOG_LOAD)); */
}
...
void mpcore_wdt_set_counter(unsigned long counter_value)
{
	...
	spin_unlock_irqrestore(&wdt_lock, flags);
	printk(KERN_DEBUG "WDT mpcore_wdt_set_counter:cpu-%d,counter_value: %ld, TWD_WDOG_COUNTER: 0x%x\n", cpu, counter_value, readl(mp_wdt.base + TWD_WDOG_COUNTER));
	/* mpcore_wdt_print("mpcore_wdt_get_counter:cpu-%d,counter_value: 0x%x\n", cpu, counter_value); */
}
...

{
	...
	/* mtk_wdt_disable(); */
	wdt.base = (void __iomem *)0xf000a600;
	printk(KERN_ERR "wdt_irq control: 0x%x, count:0x%x, load:0x%x\n", readl(wdt.base + TWD_WDOG_CONTROL), readl(wdt.base + TWD_WDOG_COUNTER), readl(wdt.base + TWD_WDOG_LOAD));
	printk(KERN_ERR "wdt_irq Triggered :cpu-%d\n", cpu);
	if (readl(wdt.base + TWD_WDOG_INTSTAT)) {
/* __inc_irq_stat(cpu, local_timer_irqs); */
		writel(1, wdt.base + TWD_WDOG_INTSTAT);

	}
	printk(KERN_ERR "wdt_irq exit :cpu-%d\n", cpu);
	aee_wdt_irq_info();
     /* mt_trace_ISR_end(30); */
...
static void irq_wdt_init(void)
{
	...
	{
	      printk(KERN_DEBUG "fwq user IRQ\n");
	}
	return;
out_free:
	 printk(KERN_ERR "fwq error\n");
	free_percpu(wdt_evt);
	return;
...
/* struct resource *res; */
	...
	ret = request_irq(wdt->irq, mpcore_wdt_fire, IRQF_TRIGGER_HIGH,
							"mpcore_wdt", wdt);
	if (ret) {
		printk(KERN_ERR DEBUG_INFO"cannot register IRQ%d for watchdog\n", wdt->irq);
		goto err_irq;
	}
#endif
	...
	printk(KERN_DEBUG DEBUG_INFO"mpcore wdt probe success\n");
	return 0;
...
int mpcore_wk_wdt_config(enum wk_wdt_type type, enum wk_wdt_mode mode, int timeout_val)
{
	/* mpcore_wdt_print("enter:type:%d, mode:%d, val:%d\n", type, mode, timeout_val); */
      printk(KERN_DEBUG "WDT enter:type:%d, mode:%d, val:%d\n", type, mode, timeout_val);
...
static int __init mpcore_wdt_init(void)
{
	...
	printk(KERN_DEBUG "MPcore Watchdog Timer: 0.1. "
	...
	if (ret) {
		printk(KERN_DEBUG DEBUG_INFO"Unable to device register(%d)\n", ret);
		return ret;
	}

	ret = platform_driver_register(&mpcore_wdt_driver);
	if (ret) {
		printk(KERN_DEBUG DEBUG_INFO"Unable to driver register(%d)\n", ret);
		return ret;
	}
	printk(KERN_DEBUG DEBUG_INFO"MPcore init\n");
	return 0;
}
